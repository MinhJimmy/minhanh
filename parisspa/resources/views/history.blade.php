                            ...
                            <tbody>
                                @foreach ($orders['all'] as $order)
                                    <tr class="product-data">
                                        ...
                                        {{-- <td class="first-row">{{ $order->user->fullname }}</td>
                                        <td class="first-row">{{ $order->user->email }}</td> --}}
                                        <td class="first-row">{{ $order->phone }}</td>
                                        <td class="first-row">{{ $order->address }}</td>
                                        <td class="first-row">{{ ($order->payment_method == 1) ? 'COD': 'Paypal' }}</td>
                                        <td class="first-row" style="color: {{ ($order->status == 1) ? 'goldenrod' : (($order->status == 2) ? 'lightseagreen' : 'lightcoral') }}">
                                            {{ ($order->status == 1) ? 'Đang vận chuyển': (($order->status == 2) ? 'Đã hoàn thành' : 'Đã huỷ') }}
                                        </td>
                                        <td class="first-row">{{ $order->created_at }}</td>
                                        <td class="first-row"><a href="/orders/detail/{{ $order->id }}" style="color: #F9A392; text-decoration: underline">Chi tiết</a></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
